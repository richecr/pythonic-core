...
w wheel:
	cp setup_gopy.py pythonic_core/setup.py
	${PYTHON_BINARY_PATH} -m pip install pybindgen
	CGO_LDFLAGS_ALLOW=".*" gopy pkg -name=pythonic_core -rename=true -output="_tmp/${PACKAGE_NAME}" -vm=${PYTHON_BINARY_PATH} github.com/richecr/pythonic_core github.com/richecr/pythonic_core/lib/pythonic github.com/richecr/pythonic_core/lib/query github.com/richecr/pythonic_core/lib/dialects
...
curl_go:
	[ ! -f "go.tar.gz" ] && curl -o go.tar.gz "https://dl.google.com/go/go1.21.4.linux-amd64.tar.gz" || echo "found go tar"
